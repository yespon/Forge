  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-8">
        <h1 className="text-3xl font-bold mb-8">{t('skills.title')}</h1>
        <p className="text-muted-foreground mb-6">
          {t('skills.total', { count: data?.total || 0 })}
        </p>
                ...
                <div className="flex items-center gap-2 text-sm">
                  {skill.is_active ? (
                    <span className="flex items-center gap-1 text-green-600">
                      <CheckCircle className="h-4 w-4" /> {t('skills.active')}
                    </span>
                  ) : (
                    <span className="flex items-center gap-1 text-gray-400">
                      <XCircle className="h-4 w-4" /> {t('skills.inactive')}
                    </span>
                  )}
                  {skill.has_instructions && (
                    <span className="text-xs px-2 py-1 bg-blue-100 text-blue-800 rounded">
                      {t('skills.has_instructions')}
                    </span>
                  )}
                </div>
              </CardContent>
